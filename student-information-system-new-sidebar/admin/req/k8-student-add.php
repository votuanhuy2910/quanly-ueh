    ...
    if ($_SESSION['role'] == 'Admin') {
        ...
        if (
            ...
            if (empty($admission_num)) {
                ...
                header("Location: ../k8-student-add.php?error=$em&$data");
                exit;
            }
            ...
            $sql = "SELECT student_id FROM students8 WHERE profile_num = ?";
            ...
            $sql = "SELECT student_id FROM students8 WHERE mssv = ?";
            $stmt = $conn->prepare($sql);
            $stmt->execute([$mssv]);
            ...
            if (empty($fname)) {
                ...
                $sql  = "INSERT INTO
                students8(admission_num, profile_num, mssv, fname, date_of_birth, gender, course, course_ori, status, note, note_special)
                VALUES(?,?,?,?,?,?,?,?,?,?,?)";
                $stmt = $conn->prepare($sql);
                ...
                header("Location: ../k8-student.php?success=$sm");
                exit;
            }
        } else {
            $em = "Xảy ra lỗi!";
            header("Location: ../k8-student-add.php?error=$em");
            exit;
        }
    } else {
